mgQuick <- function(genD, coords, longlat = FALSE, truncation = NULL, transformation = NULL,
    ...
    if (longlat) {
      ...
      eucDist <- as.matrix(geosphere::distm(coords, fun = geosphere::distMeeus))
    ...
    if (ncol(memSelected) == 0) {
      ...
      return(list(
        error = simpleError("No significant Moran's eigenvectors"),
        P = NA,
        RsqAdj = 0,
        F = NA,
        memgene = NA,
